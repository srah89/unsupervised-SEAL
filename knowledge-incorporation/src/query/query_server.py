...
- Training datasets (with "completions"/"completion" fields): Fine-tuning enabled
- Evaluation datasets (questions only): Baseline evaluation only
...
def get_completions_from_item(item: Dict[str, Any], k_completions: int) -> List[str]:
    """
    Extract completions from a dataset item, handling both training and evaluation datasets.
    
    Returns:
        - Training datasets: List of actual completions (up to k_completions)
        - Evaluation datasets: Single empty completion [""] for baseline-only evaluation
    """
    # Check if this is a training dataset (has completion data)
    if "completions" in item or "completion" in item:
        # Training mode: Extract and limit completions
        completions = (
            item["completions"] if isinstance(item.get("completions"), list)
            else [item.get("completion", "")]
        )
        # Filter out empty completions and limit to k_completions
        return [c for c in completions if c.strip()][:k_completions]
    else:
        # Evaluation mode: Single empty completion for baseline evaluation
        return [""]
...
def main() -> None:
    ...
    dataset_type = "training" if ("completions" in dataset[0] or "completion" in dataset[0]) else "evaluation"
    print(f"Detected {dataset_type} dataset: {data_path}")

    # ---------------- iterate over articles -------------------------- #
    for art_idx, item in enumerate(dataset):
        completions = get_completions_from_item(item, args.k_completions)
